    ...
    public class AzureADSearchEngine : ISearchEngine
    {
        ...
        public async Task<IEnumerable<SearchEngineResultDto>> SearchContactAsync(string searchQuery, string delegatedToken, ClaimsPrincipal userPrincipal)
        {
            ...
            try
            {
                ...
                {
                    requestConfiguration.QueryParameters.Filter = AZURE_AD_GRAPH_FILTER_TEMPLATE.Replace(SEARCH_QUERY_PLACEHOLDER, ODataFilterHelper.EscapeODataStringLiteral(searchQuery)).Replace(ENTRA_ID_FILTER, entraIdFilterString.ToString());
